...
finalNode = b'\xaa'  # Destination radio
origNode = b'\xab'  # Current radio. Changes depending on the radio
    ...
    if response is not None:
        ...
        for ID in seendID:
            if response[2] == ID:  # we have seen this message. We use single numbers to access the int value of the byte
                if response[6:7] == yesSatAck:  # its our ack! We use number:number to access the string of the byte
